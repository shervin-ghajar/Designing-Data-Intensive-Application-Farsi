...
> همه‌چیز تغییر می‌کند و هیچ‌چیز ثابت نمی‌ماند. — هراکلیتوس

applicationها در طول زمان ناچار تغییر می‌کنند. feature تازه‌ای اضافه می‌شود، feature قدیمی شکل دیگری پیدا می‌کند، محصول جدیدی عرضه می‌شود یا نیاز کسب‌وکار بهتر فهمیده می‌شود. اگر سیستم را طوری بسازیم که تغییر دادن آن آسان باشد، به `evolvability` رسیده‌ایم.
...
- `backward compatibility`: code جدید بتواند داده‌ای را که code قدیمی نوشته است بخواند.
- `forward compatibility`: code قدیمی بتواند داده‌ای را که code جدید نوشته است بخواند.

backward compatibility معمولاً ساده‌تر است، چون نویسندهٔ code جدید قالب قدیمی را می‌شناسد و می‌تواند آن را صریحاً پشتیبانی کند. forward compatibility سخت‌تر است؛ code قدیمی باید بتواند additionهای ناشناختهٔ نسخهٔ جدید را نادیده بگیرد.
...
- versioning و forward/backward compatibility معمولاً بعداً و به‌صورت ناقص به library اضافه می‌شود.
- CPU موردنیاز و حجم encoded data گاهی نادیده گرفته می‌شود؛ serialization داخلی Java به پرحجمی و performance ضعیف معروف است.
...
JSON و XML برای stringهای Unicode خوب‌اند، اما دنبالهٔ byte بدون encoding کاراکتری را مستقیماً ندارند. راه رایج تبدیل binary به text با `Base64` است. این راه کار می‌کند، ولی اندازهٔ داده را حدود ۳۳٪ زیاد می‌کند و schema باید بگوید این string در واقع Base64 است.

### schema اختیاری و CSV مبهم

XML Schema و JSON Schema امکان validation دقیق می‌دهند، اما خودشان پیچیده‌اند و یادگیری و پیاده‌سازی‌شان هزینه دارد. اگر schema استفاده نشود، application باید نوع و معنای fieldها را hardcode کند.

CSV اصلاً schema ندارد؛ معنی هر row و column را application تعیین می‌کند. با اضافه شدن column یا row باید code دستی تغییر کند. همچنین اگر مقدار شامل comma یا newline باشد، parser و escaping اهمیت پیدا می‌کند و همهٔ parserها رفتار کاملاً یکسانی ندارند.
...
`Apache Thrift` و `Protocol Buffers` یا `protobuf` بر اصل مشترکی بنا شده‌اند: schema لازم است و از روی schema code تولید می‌شود. Protocol Buffers ابتدا در Google و Thrift ابتدا در Facebook ساخته شد و هر دو بعداً open source شدند.

schema رکورد نمونه در Thrift:
...
Thrift چند binary protocol دارد. `BinaryProtocol` رکورد نمونه را حدود ۵۹ byte می‌کند:
...
Protocol Buffers هم رکورد را با روش مشابهی در حدود ۳۳ byte ذخیره می‌کند:
...
`required` و `optional` روی شکل binary اثر مستقیمی ندارند. `required` فقط یک check در runtime ایجاد می‌کند که اگر field مقداردهی نشده باشد failure بدهد. در نتیجه بیشتر برای پیدا کردن bug مفید است.

## field tag و schema evolution

record encoded از کنار هم قرار گرفتن fieldهای encoded ساخته شده است. هر field با tag number و datatype خود شناخته می‌شود. اگر field مقدار نداشته باشد، اصلاً در record نمی‌آید.
...
library Avro هر دو schema را کنار هم می‌گذارد و تفاوت‌ها را resolve می‌کند:

- جابه‌جا شدن ترتیب fieldها مشکلی ندارد، چون fieldها با name match می‌شوند.
- field موجود در writer اما غایب در reader نادیده گرفته می‌شود.
- field موردانتظار reader که در writer وجود ندارد با default مقداردهی می‌شود.
...
### writer’s schema را از کجا بیاوریم؟
...
- **database با recordهای مستقل:** recordها در زمان‌های مختلف با schemaهای مختلف نوشته می‌شوند. می‌توان version number را ابتدای هر record گذاشت و schema versionها را در database نگه داشت. reader version را می‌خواند و writer’s schema مناسب را پیدا می‌کند.
- **network connection:** دو process هنگام setup نسخهٔ schema را مذاکره می‌کنند و در طول connection همان را استفاده می‌کنند. Avro RPC چنین روشی دارد.

schema registry علاوه بر decode، documentation و check compatibility قبل از deploy را هم فراهم می‌کند. version می‌تواند integer افزایشی یا hash schema باشد.

### schemaهای dynamically generated

Avro چون tag number ندارد، برای schemaهای تولیدشده به‌صورت dynamic مناسب است. مثلاً اگر بخواهیم محتوای relational database را به فایل binary dump کنیم، می‌توان از schema table و columnها یک Avro schema JSON ساخت، هر table را record گرفت و هر column را field نام‌گذاری کرد.
...
### code generation و languageهای dynamic

Thrift و Protocol Buffers با code generation برای languageهای statically typed مانند Java، C++ و C# مفیدند؛ type checking و autocompletion و ساختارهای حافظه‌ای کارآمد به‌دست می‌آید. در JavaScript، Ruby یا Python که compile-time type checker ندارند، تولید code الزاماً سودی ندارد و حتی یک مرحلهٔ اضافه است.

Avro code generation را اختیاری می‌کند. اگر object container file شامل writer’s schema باشد، می‌توان آن را بدون تولید class باز کرد؛ فایل self-describing است. این ویژگی برای ابزارهای dynamically typed مانند `Apache Pig` مناسب است: فایل Avro را باز می‌کنید، تحلیل می‌کنید و dataset مشتق‌شده را دوباره در Avro می‌نویسید.

## فایدهٔ schema

schema languageهای Thrift، Protocol Buffers و Avro از XML Schema و JSON Schema ساده‌ترند؛ schemaهای XML/JSON می‌توانند ruleهایی مانند regex یا بازهٔ عددی تعریف کنند، اما پیچیدگی بیشتری دارند. سادگی این binary schemaها باعث شده library آن‌ها برای languageهای زیادی نوشته شود.
...
- schema documentation قابل‌اعتماد است؛ چون برای decode لازم است و نمی‌تواند خیلی از واقعیت داده عقب بماند.
- schema registry اجازه می‌دهد compatibility قبل از deploy بررسی شود.
...
## Dataflow از طریق database

processی که در database می‌نویسد، data را encode می‌کند و processی که بعداً می‌خواند decode. حتی اگر فقط یک application به database دسترسی داشته باشد، می‌توان ذخیره کردن data را مانند فرستادن پیام به future version خودمان دید؛ پس backward compatibility ضروری است.
...
## Dataflow از طریق Service: `REST` و `RPC`
...
client فقط browser نیست؛ native app موبایل یا desktop و JavaScript داخل browser هم می‌توانند با `XMLHttpRequest` یا Ajax به HTTP client تبدیل شوند. response ممکن است HTML نباشد، بلکه JSONی باشد که code سمت client آن را پردازش می‌کند.

server خودش می‌تواند client یک service دیگر باشد؛ مثلاً web app server از database request کند. شکستن application بزرگ به serviceهای مستقل با این الگو به `service-oriented architecture` یا امروزه `microservices architecture` نزدیک است.

service مانند database امکان submit و query می‌دهد، اما برخلاف database اجازهٔ query دلخواه نمی‌دهد؛ API فقط input و outputهایی را می‌پذیرد که business logic تعریف کرده است. این محدودیت encapsulation ایجاد می‌کند. هدف microservices این است که هر service مستقل deploy و evolve شود، بنابراین old و new client و server باید مدتی با هم کار کنند.

### web service

وقتی transport پروتکل HTTP باشد، به آن web service می‌گوییم. کاربردهای رایج:

1. application روی device کاربر که از طریق اینترنت به service request می‌دهد.
2. serviceای که به service دیگرِ همان organization، معمولاً در همان datacenter، request می‌فرستد.
3. service یک organization که به service organization دیگر request می‌دهد؛ مانند payment provider یا OAuth.
...
`SOAP` پروتکلی XML-based برای request شبکه‌ای است. هرچند بیشتر روی HTTP اجرا می‌شود، هدفش وابسته نبودن به HTTP و استفاده نکردن از بیشتر featureهای آن است. در عوض مجموعهٔ بزرگی از استانداردهای WS-* دارد.

API یک SOAP service با `WSDL` یا `Web Services Description Language` توصیف می‌شود. WSDL code generation را ممکن می‌کند تا client به‌جای ساختن XML، class و method محلی صدا بزند. این روش برای languageهای statically typed مفید است، اما WSDL برای انسان خوانا نیست و SOAP به tool و IDE وابستگی دارد. تفاوت پیاده‌سازی vendorها هم گاهی interoperability را دشوار می‌کند. SOAP در enterpriseهای بزرگ هنوز دیده می‌شود، اما REST در بسیاری از کاربردها غالب شده است.

RESTful API معمولاً ساده‌تر است و به code generation کمتر نیاز دارد. formatهایی مانند `OpenAPI` یا `Swagger` می‌توانند API را توصیف و مستندات تولید کنند.
...
- function محلی معمولاً بر اساس parameterهای تحت کنترل ما predictable است. network ممکن است request یا response را از دست بدهد، machine مقصد کند یا unavailable باشد و این‌ها خارج از کنترل caller است؛ بنابراین retry لازم می‌شود.
- function محلی result می‌دهد، exception می‌اندازد یا هیچ‌وقت برنمی‌گردد. network outcome چهارمی هم دارد: `timeout` بدون نتیجه. در این حالت نمی‌دانیم request اصلاً به مقصد رسید یا نه.
...
- client و server ممکن است با languageهای متفاوت نوشته شده باشند و RPC باید datatypeها را ترجمه کند. همهٔ languageها typeهای یکسان ندارند؛ مشکل numberهای بزرگ در JavaScript نمونه‌ای از این تفاوت است.

پس نباید remote service را بیش از حد شبیه object محلی جلوه داد. REST دست‌کم network بودن ارتباط را پنهان نمی‌کند.

### جهت‌های جدید RPC

RPC از بین نرفته است. `Thrift` و `Avro` پشتیبانی RPC دارند، `gRPC` روی Protocol Buffers است، `Finagle` از Thrift و `Rest.li` از JSON روی HTTP استفاده می‌کند.

نسل جدید صریح‌تر می‌گوید remote call با local call فرق دارد:

- `futures` یا promiseها عملیات asynchronous و failureپذیر را مدل می‌کنند و اجرای موازی requestها را ساده می‌سازند.
- gRPC از stream پشتیبانی می‌کند؛ ارتباط می‌تواند مجموعه‌ای از request و response در طول زمان باشد، نه یک جفت منفرد.
- برخی frameworkها `service discovery` دارند تا client IP و port service را پیدا کند.

RPC binary سفارشی ممکن است از JSON روی REST سریع‌تر باشد، اما RESTful API برای experimentation و debugging عالی است: با browser یا `curl` می‌توان آن را امتحان کرد، تقریباً همهٔ languageها پشتیبانی‌اش می‌کنند و ecosystem بزرگی از server، cache، load balancer، proxy، firewall، monitoring و testing دارد.

به همین دلیل REST معمولاً برای public API مناسب‌تر است و RPC بیشتر درون یک organization و datacenter استفاده می‌شود.

### encoding و evolution در RPC
...
- Thrift، gRPC و Avro RPC قواعد format خودشان را دارند.
- SOAP از XML schema استفاده می‌کند؛ evolution ممکن است، ولی ظرافت‌های زیادی دارد.
- REST معمولاً JSON بدون schema رسمی دارد. افزودن request parameter اختیاری یا field جدید به response معمولاً compatibility را حفظ می‌کند.
...
client message را با latency کم به process دیگر می‌فرستد، اما پیام از connection مستقیم عبور نمی‌کند؛ واسطه‌ای به نام `message broker`، `message queue` یا `message-oriented middleware` آن را موقتاً ذخیره می‌کند.

message broker چند مزیت دارد:
...
- sender لازم نیست IP و port recipient را بداند؛ در cloud که machineها مرتب جابه‌جا می‌شوند مهم است.
- یک message را می‌توان به چند recipient فرستاد.
- sender و recipient از نظر منطقی decouple می‌شوند؛ producer فقط publish می‌کند و لازم نیست مصرف‌کننده را بشناسد.
...
`RabbitMQ`، `ActiveMQ`، `HornetQ`، `NATS` و `Apache Kafka` نمونه‌های رایج‌اند. معمولاً producer message را به queue یا `topic` نام‌دار می‌فرستد و broker تحویل آن را به یک یا چند consumer یا subscriber مدیریت می‌کند. یک topic می‌تواند producer و consumerهای زیادی داشته باشد.

topic ذاتاً one-way است، اما consumer می‌تواند پیام را به topic بعدی publish کند و pipeline بسازد، یا به reply queue بفرستد تا sender اولیه آن را بخواند.

broker معمولاً data model خاصی enforce نمی‌کند؛ message دنباله‌ای از byte با metadata است، پس JSON، Avro، protobuf یا هر format دیگری ممکن است. اگر encoding forward/backward-compatible باشد، publisher و consumer را می‌توان مستقل و با ترتیب‌های متفاوت deploy کرد. اگر consumer پیام را دوباره publish می‌کند، باید fieldهای ناشناخته را حفظ کند تا مانند مشکل database داده دور ریخته نشود.

## distributed actor framework

`actor model` مدلی برای concurrency در یک process است. به‌جای کار مستقیم با thread و race condition و lock و deadlock، منطق در actorها encapsulate می‌شود. هر actor معمولاً یک entity یا client است، state محلی خودش را دارد و با messageهای asynchronous با actorهای دیگر حرف می‌زند.
...
- `Akka` به‌طور پیش‌فرض از Java serialization استفاده می‌کند که forward/backward compatibility خوبی ندارد؛ می‌توان آن را با protobuf جایگزین کرد.
- `Orleans` format سفارشی‌ای دارد که rolling upgrade را پیش‌فرض پشتیبانی نمی‌کند؛ معمولاً cluster جدید ساخته و traffic منتقل می‌شود.
...
- encoding مخصوص یک language ساده است، اما به همان language محدود می‌شود و معمولاً compatibility ضعیفی دارد.
- JSON، XML و CSV گسترده و قابل‌خواندن‌اند؛ compatibility آن‌ها به نحوهٔ استفاده بستگی دارد و دربارهٔ number و binary string ابهام دارند.
- Thrift، Protocol Buffers و Avro schema-driven و فشرده‌اند و قواعد compatibility روشن‌تری دارند. schema documentation و code generation مفید است، اما برای خواندن انسان باید decode شوند.
...
`encoding` تبدیل object یا data structure در memory به دنبالهٔ byte برای فایل یا network است. `decoding` تبدیل byte به representation قابل‌استفاده در برنامه است. این دو با encryption یکی نیستند.
...
code یا schema جدید بتواند data نوشته‌شده با نسخهٔ قدیمی را بخواند.

### `forward compatibility`

code یا schema قدیمی بتواند data نوشته‌شده با نسخهٔ جدید را بخواند؛ معمولاً با نادیده گرفتن fieldهای ناشناخته.
...
deploy تدریجی version جدید روی nodeها، بدون خاموش کردن کل service. در این مدت old و new code هم‌زمان فعال‌اند.
...
`Remote Procedure Call`؛ فراخوانی یک operation روی process دیگر از طریق network. باید timeout، retry، latency متغیر و احتمال اجرای دوباره را صریحاً مدیریت کند.
