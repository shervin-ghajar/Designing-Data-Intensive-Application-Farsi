...
**Consensus** یعنی چند node روی یک تصمیم مشترک توافق کنند. مثلاً اگر leader database از دست برود، nodeهای باقی‌مانده باید دقیقاً روی یک leader جدید توافق کنند؛ وگرنه دو node ممکن است خودشان را leader بدانند و **split brain** و data loss رخ دهد. پیش از رسیدن به الگوریتم‌های consensus، ابتدا باید بفهمیم consistency چه guaranteeهایی دارد و ترتیب رویدادها چگونه تعریف می‌شود.

## Guaranteeهای **Consistency**
...
stronger guaranteeها استفاده را ساده‌تر می‌کنند، اما رایگان نیستند: ممکن است latency و هزینهٔ coordination را بالا ببرند یا هنگام partition دسترس‌پذیری را کم کنند. **transaction isolation** و consistency توزیع‌شده نیز یکی نیستند:

- isolation دربارهٔ جلوگیری از race میان transactionهای concurrent داخل یک database است؛
- distributed consistency دربارهٔ هماهنگ‌کردن replicaها در برابر delay و fault است.

سه موضوع این فصل به هم پیوند دارند:

1. **linearizability**، یکی از قوی‌ترین modelهای معمول consistency؛
2. **causality** و روش‌های مرتب‌کردن eventها؛
3. **distributed transaction**، atomic commit و در نهایت consensus.
...
- **read(x) ⇒ v**: مقدار x خوانده شده و v برگشته است.
- **write(x, v) ⇒ r**: x روی v تنظیم شده و پاسخ r، مانند **ok** یا **error**، برگشته است.

فرض کنید مقدار اولیهٔ x صفر است و client C می‌خواهد آن را به یک تغییر دهد. clientهای A و B هم‌زمان مرتب x را می‌خوانند:

- readای که پیش از شروع write تمام شده، حتماً باید صفر برگرداند.
- readای که بعد از پایان write شروع شده، حتماً باید یک برگرداند.
- readای که با write overlap دارد، می‌تواند صفر یا یک ببیند، چون نقطهٔ اتمیک‌شدن write در داخل بازهٔ آن معلوم نیست.
...
**cas(x, old, new) ⇒ r** مقدار x را فقط وقتی به new تغییر می‌دهد که مقدار فعلی دقیقاً old باشد؛ در غیر این صورت error برمی‌گرداند و x بدون تغییر می‌ماند.

هر operation باید بتواند در یک ترتیب متوالی معتبر قرار بگیرد:

- read مقدار آخرین write قبل از خودش را ببیند؛
- compare-and-set فقط وقتی موفق شود که شرطش در آن نقطه برقرار باشد؛
...
- response یک write ممکن است دیرتر از read دیگری برسد؛ این به معنی خوانده‌شدن قبل از write نیست.
- linearizability به‌تنهایی transaction isolation نیست. ممکن است client دیگری بین دو read شما مقدار را تغییر دهد.
- compare-and-set برای جلوگیری از تغییر هم‌زمان مفید است، اما باید خود operation اتمیک و linearizable باشد.
...
### Linearizability در برابر **Serializability**
...
### constraint و uniqueness

username، email، path و filename معمولاً باید یکتا باشند. اگر دو نفر هم‌زمان یک username بسازند، باید یکی موفق و دیگری با constraint violation شکست بخورد. این شبیه **compare-and-set** است: username را فقط اگر آزاد است به user ID تنظیم کن.
...
linearizability ساده‌ترین راه جلوگیری از این race است: وقتی پیام queue رسیده، read بعدی باید write تصویر را ببیند. روش‌های دیگری مانند انتقال version، انتظار برای read-your-writes یا قرار دادن metadata در پیام هم ممکن‌اند، اما پیچیده‌ترند.
...
- asynchronous replication ممکن است در failover write commitشده را از دست بدهد؛ این هم durability و هم linearizability را نقض می‌کند.
- snapshot isolation یا bugهای concurrency ممکن است semantics را ضعیف کنند.

### consensus

consensus protocolها برای جلوگیری از split brain و استفاده از replica قدیمی، جزئیات بیشتری نسبت به replication ساده دارند. ZooKeeper و etcd از همین راه storage linearizable می‌سازند.

### multi-leader و leaderless

multi-leader معمولاً linearizable نیست، چون چند node هم‌زمان write می‌پذیرند و conflict بعداً حل می‌شود. در مدل Dynamo-style نیز quorum read و write با شرط **w + r > n** شهوداً قوی به نظر می‌رسد، اما strict quorum به‌تنهایی کافی نیست.
...
linearizability حتی روی memory چند هسته‌ای CPU هم بدون memory barrier تضمین نمی‌شود. کنارگذاشتن linearizability اغلب performance و availability را بهتر می‌کند، نه اینکه به‌خودی‌خود fault tolerance را بیشتر کند. اگر کاربر بتواند به channel دیگری مانند log، email یا conversation دسترسی داشته باشد، inconsistency ممکن است آشکار شود.

## Guaranteeهای **Ordering**

linearizable register گویی یک copy دارد و هر operation در یک نقطهٔ اتمیک اعمال می‌شود؛ بنابراین همهٔ operationها ترتیب مشخصی دارند. این موضوع با چند مفهوم دیگر مرتبط است:

- leader در single-leader replication ترتیب writeهای log را تعیین می‌کند؛
- serializability transactionها را مانند یک ترتیب متوالی می‌کند؛
- timestampها می‌کوشند بگویند کدام event دیرتر رخ داده است.

### ترتیب و **Causality**

**Causality** رابطهٔ علت و معلول است:

- سؤال باید پیش از پاسخ وجود داشته باشد.
- row باید پیش از update آن ایجاد شده باشد.
...
- snapshot سازگار باید علت را همراه اثر آن ببیند؛ read skew می‌تواند حالتی بسازد که effect حاضر است ولی cause غایب.
- write skew نیز از مشاهدهٔ قبلی state و تصمیم بعدی ناشی می‌شود؛ SSI با دنبال‌کردن dependencyها آن را تشخیص می‌دهد.
...
### partial order در برابر total order
...
- timestamp ساعت تقویمی را به operation بچسبانیم؛ clock skew ممکن است operation دیرتر را کوچک‌تر کند.
- blockهای شماره را از پیش رزرو کنیم؛ node A شمارهٔ ۱ تا ۱۰۰۰ و B شمارهٔ ۱۰۰۱ تا ۲۰۰۰ را بگیرد. operation causal بعدی ممکن است هنوز از block کوچک‌تر استفاده کند.
...
Lamport timestamp یک جفت **(counter, node ID)** است. هر node counter خودش را نگه می‌دارد و node ID یکتا باعث می‌شود timestampها برابر نباشند. قاعدهٔ اصلی این است:
...
Lamport timestamp را با **version vector** یکی ندانید:

- version vector می‌تواند concurrentبودن دو operation را از causalبودن آن‌ها تشخیص دهد؛
- Lamport timestamp همیشه یک total order تحمیل می‌کند و از روی دو عدد نمی‌توان فهمید دو operation واقعاً concurrent بوده‌اند یا نه.
...
- **Reliable delivery:** اگر پیامی به یک node تحویل شد، به همهٔ nodeها تحویل می‌شود؛ پیام گم نمی‌شود.
- **Totally ordered delivery:** همهٔ nodeها پیام‌ها را دقیقاً در یک ترتیب می‌بینند.
...
- total order broadcast مانند یک append-only log، transaction log یا WAL مشترک عمل می‌کند.
...
- position آخرین log را با روش linearizable گرفت و صبر کرد همهٔ entryهای قبل از آن روی replica اعمال شوند؛
...
## **Distributed Transactions** و **Consensus**

Consensus از بنیادی‌ترین مسئله‌های distributed computing است. مثال‌های آن:
...
1. application یک transaction ID سراسری و یکتا می‌گیرد.
2. read و writeهای هر participant با همان ID انجام می‌شوند.
3. coordinator به همهٔ participantها **prepare** می‌فرستد.
4. participant همهٔ داده‌ها را durable می‌کند، conflict و constraint را بررسی می‌کند و اگر پاسخ **yes** دهد، قول می‌دهد بعداً بدون خطا commit کند.

**Phase 2: commit یا abort**

5. اگر همه **yes** گفتند، coordinator تصمیم commit می‌گیرد؛ اگر حتی یکی **no** گفت یا prepare timeout شد، تصمیم abort است.
6. coordinator تصمیم را پیش از اعلام به participantها در log خودش durable می‌کند؛ این لحظه **commit point** است.
...
- timeout به‌تنهایی کمک نمی‌کند؛ abort یک participant ممکن است با commit participant دیگر ناسازگار شود.
- commit یک‌طرفه هم خطرناک است، چون شاید participant دیگری abort کرده باشد.
...
## distributed transaction در عمل
...
participantها فناوری‌های متفاوت‌اند: دو database از vendorهای مختلف، database و message broker یا حتی سرویس دیگری. هماهنگ‌کردن آن‌ها سخت‌تر است و معمولاً با **XA** انجام می‌شود.
...
**XA** مخفف **eXtended Architecture** و استانداردی برای 2PC میان فناوری‌های مختلف است. XA خود network protocol نیست؛ یک API برای ارتباط driver با coordinator است. در Java معمولاً از **JTA** برای database driverهای **JDBC** و message brokerهای **JMS** استفاده می‌شود.

coordinator ممکن است به‌صورت library داخل همان application process باشد. اگر process یا machine آن crash کند، log coordinator و transactionهای prepareشده نیز در دسترس نیستند؛ participantها نمی‌توانند مستقیم coordinator را پیدا کنند و در وضعیت in-doubt باقی می‌مانند.

### lockهایی که در وضعیت in-doubt نگه داشته می‌شوند

participant نمی‌تواند lockهای transaction را آزاد کند، چون آزادکردن آن‌ها پیش از تصمیم نهایی ممکن است dirty write یا atomicity violation بسازد. بنابراین اگر coordinator بیست دقیقه down باشد، rowهای تغییرکرده ممکن است بیست دقیقه lock بمانند. اگر log برای همیشه از دست برود، lockها نیز تا دخالت administrator باقی می‌مانند و بخش بزرگی از application unavailable می‌شود.
...
- coordinatorی که روی یک machine و بدون replication است، single point of failure کل سیستم می‌شود.
- application server دیگر واقعاً stateless نیست، چون log coordinator بخشی از durable state حیاتی است.
- XA lowest common denominator است و نمی‌تواند همهٔ قابلیت‌های خاص databaseها مانند deadlock detection میان چند system یا SSI را حفظ کند.
...
- **Uniform agreement:** هیچ دو nodeای متفاوت تصمیم نگیرند.
- **Integrity:** هیچ nodeای دوبار تصمیم نگیرد.
- **Validity:** value انتخاب‌شده واقعاً از سوی یک node پیشنهاد شده باشد.
- **Termination:** هر nodeای که crash نکرده، سرانجام تصمیم بگیرد.
...
### consensus و total order broadcast
...
- agreement باعث می‌شود همهٔ nodeها همان messageها را به همان ترتیب ببینند؛
- integrity جلوی duplicate را می‌گیرد؛
- validity جلوی message ساختگی یا تغییرکرده را می‌گیرد؛
- termination باعث می‌شود message گم نشود.
...
### single-leader replication و مسئلهٔ مرغ و تخم‌مرغ

single-leader replication ظاهراً total order broadcast است: همهٔ writeها به leader می‌روند و followerها آن‌ها را به همان ترتیب اعمال می‌کنند. اگر operator دستی leader را تعیین کند، سیستم برای write تا recover یا reconfigure دستی leader متوقف می‌شود؛ این روش safety می‌تواند داشته باشد، اما termination consensus را ندارد.
...
leader برای هر تصمیم:

1. epoch خود را به nodeها اعلام می‌کند و از quorum برای election رأی می‌گیرد؛
2. proposal را می‌فرستد و از quorum رأی موافق می‌خواهد؛
3. node فقط وقتی رأی می‌دهد که leader بزرگ‌تری نشناسد.

quorum مرحلهٔ election و quorum مرحلهٔ proposal باید overlap داشته باشند. در نتیجه اگر proposal رأی گرفت و epoch بزرگ‌تری در پاسخ‌ها دیده نشد، leader می‌فهمد election جدیدی که بر ضد او باشد رخ نداده است.
...
consensus safety قوی و total order می‌دهد، اما هزینه دارد:
...
- membership پویا از membership ثابت پیچیده‌تر و کمتر آزموده‌شده است.
- timeoutهای متغیر می‌توانند leader را بی‌دلیل dead فرض کنند و electionهای مکرر performance را نابود کنند.
...
### operation اتمیک linearizable

compare-and-set روی یک key می‌تواند lock بسازد. اگر چند node هم‌زمان تلاش کنند، فقط یکی موفق می‌شود. lock معمولاً به شکل lease با expiry پیاده می‌شود تا در صورت crash client آزاد شود. برای جلوگیری از اثر process قدیمی، sequence number عملیات می‌تواند fencing token باشد.

### ترتیب کل operationها

ZooKeeper هر operation را در order کلی قرار می‌دهد و **zxid** و **cversion** افزایشی می‌دهد. این شماره‌ها برای fencing، replay و تشخیص نسل تغییرات مفیدند.

### failure detection و session

client و server heartbeat می‌فرستند و session بلندمدت دارند. قطع موقت connection لزوماً session را فوراً نمی‌کشد؛ اگر heartbeat بیش از session timeout قطع شود، server session را مرده اعلام می‌کند و lockها یا **ephemeral node**های آن را آزاد می‌کند.

### change notification

client می‌تواند روی value یا membership watch بگذارد و به‌جای polling مداوم، تغییر را notification بگیرد. این برای فهمیدن join، leave، leader failure و تغییر configuration مفید است.
...
coordination service را کوچک نگه دارید: چند node ثابت برای رأی‌گیری و تعداد زیادی client. runtime state با میلیون‌ها تغییر در ثانیه را در آن نگذارید؛ آن state به ابزار replication دیگری نیاز دارد.

## جمع‌بندی فصل

**Linearizability** replicated data را شبیه یک copy واحد و operationها را اتمیک نشان می‌دهد. استفاده از آن ساده است، اما در network delay زیاد هزینه و هنگام partition محدودیت availability دارد.

**Causality** ترتیب علت و معلول را حفظ می‌کند، اما total order نیست؛ history می‌تواند branch و merge داشته باشد. causal consistency معمولاً از linearizability کم‌هزینه‌تر و در برابر network fault دسترس‌پذیرتر است.

Lamport timestamp می‌تواند یک total order سازگار با causality بسازد، اما تا وقتی order نهایی نشده، برای uniqueness و تصمیم فوری کافی نیست. **Total order broadcast** ترتیب را در لحظهٔ delivery نهایی می‌کند و برای replication، log، lock و storage مفید است.

Consensus یعنی تصمیم مشترک، غیرقابل‌برگشت و معتبر. مسئله‌های زیر در اصل به consensus کاهش‌پذیرند:
...
### **ZooKeeper**، **etcd** و **coordination service**
