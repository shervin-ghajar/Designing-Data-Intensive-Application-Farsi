...
2. **partitioned log** و تفاوت آن با message brokerهای سنتی
3. databaseها به‌عنوان منبع stream و نگه‌داشتن چند سیستم در sync
4. **Change Data Capture** و **Event Sourcing**
...
- **lazy list**ها در زبان‌های برنامه‌نویسی
- APIهای filesystem مانند FileInputStream در Java
- اتصال‌های TCP
...
event را می‌توان به‌شکل متن، JSON یا یک قالب binary ذخیره کرد. سپس همان event را می‌توان به فایل append کرد، در یک جدول relational گذاشت، در یک document database ذخیره کرد یا به node دیگری فرستاد.
...
1. **drop**: بعضی پیام‌ها حذف شوند.
2. **buffer**: پیام‌ها در queue نگه‌داری شوند تا consumer به آن‌ها برسد.
3. **backpressure** یا flow control: producer متوقف یا کند شود تا consumer عقب نماند.

pipeهای Unix و TCP معمولاً backpressure دارند. buffer آن‌ها محدود است؛ وقتی پر شود، sender تا خالی شدن بخشی از آن متوقف می‌ماند. اگر سیستم پیام‌ها را در queue ذخیره کند، باید مشخص شود queue در چه نقطه‌ای از memory به disk منتقل می‌شود و این انتقال چه اثری بر performance دارد.
...
- **UDP multicast** در feedهای بازار مالی رایج است؛ latency کم است، اما خود UDP قابل‌اعتماد نیست. اگر برنامه به پیام‌های ازدست‌رفته نیاز داشته باشد، باید شمارهٔ packetها را نگه دارد و امکان درخواست retransmission بدهد.
...
1. producer به broker متصل می‌شود و پیام را در آن می‌نویسد.
2. broker پیام را نگه می‌دارد.
3. consumer به broker متصل می‌شود و پیام را می‌خواند.
4. broker مسئولیت اصلی تحمل clientهایی را برعهده می‌گیرد که connect، disconnect یا crash می‌شوند.
...
brokerهایی با مدل سنتی مانند AMQP/JMS برای task queue و asynchronous RPC مناسب‌اند؛ یعنی جایی که پیام یک کار است و پس از پردازش دیگر نیازی به خواندن دوبارهٔ آن نیست.

### چند consumer: load balancing و fan-out

وقتی چند consumer به یک topic وصل می‌شوند، دو الگوی اصلی داریم:

**Load balancing**: هر message فقط به یکی از consumerها تحویل داده می‌شود. consumerها کار را میان خود تقسیم می‌کنند. اگر هر پیام پردازش سنگینی داشته باشد، با اضافه کردن consumer می‌توان parallelism را افزایش داد.

**Fan-out**: هر message به همهٔ consumerها تحویل داده می‌شود. در این حالت چند consumer مستقل می‌توانند یک broadcast مشترک را بخوانند؛ درست مثل چند batch job که یک فایل ورودی را جداگانه می‌خوانند.
...
- **database trigger** با هر تغییر رکوردی در changelog می‌نویسد. این روش ساده به‌نظر می‌رسد، اما می‌تواند fragile باشد و overhead داشته باشد.
- parse کردن replication log یا WAL معمولاً robustتر است، اما با schema change و جزئیات داخلی database درگیر می‌شود.
...
CDC معمولاً asynchronous است. database اصلی منتظر نمی‌ماند تا همهٔ consumerها تغییر را اعمال کنند. این کار slow consumer را از مسیر اصلی جدا می‌کند، اما replication lag همچنان وجود دارد.
...
Kafka از این قابلیت پشتیبانی می‌کند. این ویژگی broker را از یک مسیر تحویل transient به نوعی storage پایدار برای state و change stream تبدیل می‌کند.
...
**Event Sourcing** شبیه CDC است، اما در سطح دیگری انجام می‌شود:
...
- CDC معمولاً برای update یک record، نسخهٔ کامل جدید آن record را ثبت می‌کند. آخرین event برای key، مقدار فعلی را تعیین می‌کند؛ پس نسخه‌های قدیمی قابل حذف‌اند.
- event در Event Sourcing معمولاً intent کاربر را بیان می‌کند، نه کل state جدید را. event بعدی event قبلی را overwrite نمی‌کند؛ مثلاً «کالا به cart اضافه شد» با «کالا از cart حذف شد» هر دو واقعیت‌های مهمی هستند. برای بازسازی state باید history کامل را نگه داشت.
...
- command درخواست یا نیت اولیهٔ کاربر است و ممکن است رد شود.
- event واقعیتی است که تأیید شده، durable و immutable است.
...
### state، stream و immutability

state mutable و event log immutable متناقض نیستند؛ دو نگاه به یک فرایندند. فهرست صندلی‌های آزاد نتیجهٔ reservationهایی است که تاکنون پردازش شده‌اند. balance حساب نتیجهٔ credit و debitهاست. graph زمان پاسخ سرویس نتیجهٔ aggregate کردن پاسخ همهٔ requestهاست.
...
immutable eventها فقط state فعلی را ذخیره نمی‌کنند؛ اطلاعاتی را هم حفظ می‌کنند که state فعلی پنهان می‌کند. مثلاً کاربر کالایی را به cart اضافه و بلافاصله حذف کرده است. برای fulfillment نتیجه این است که کالا در cart نیست، اما برای analytics مهم است که کاربر زمانی به آن کالا علاقه نشان داده است.
...
1. eventها را در database، cache، search index یا storage دیگری بنویسیم تا clientها آن‌ها را query کنند.
2. eventها را به انسان‌ها برسانیم؛ مثلاً با email، push notification یا dashboard لحظه‌ای.
...
**Complex Event Processing** یا **CEP** برای پیدا کردن الگوهای event استفاده می‌شود. همان‌طور که regular expression الگوی خاصی از characterها را پیدا می‌کند، CEP rule مشخص می‌کند چه دنباله‌ای از eventها باید دیده شود.
...
CDC stream می‌تواند cache، search index و warehouse را با database اصلی همگام نگه دارد. این کار نمونه‌ای از **materialized view maintenance** است: یک view جایگزین برای query می‌سازیم تا خواندن سریع شود و با هر تغییر source آن را update می‌کنیم.
...
- **event time**: زمانی که event واقعاً رخ داده است.
- **processing time**: زمانی که stream processor آن را دریافت و پردازش کرده است.
...
window ثابت و بدون هم‌پوشانی است. در window یک‌دقیقه‌ای، eventهای ۱۰:۰۳:۰۰ تا ۱۰:۰۳:۵۹ در یک گروه و eventهای ۱۰:۰۴:۰۰ تا ۱۰:۰۴:۵۹ در گروه بعد قرار می‌گیرند. معمولاً timestamp را به پایین‌ترین دقیقه گرد می‌کنیم.
...
session window طول ثابتی ندارد. eventهای یک user تا وقتی فاصلهٔ آن‌ها کوتاه است در یک session قرار می‌گیرند. اگر user مثلاً سی دقیقه هیچ کاری نکند، session تمام می‌شود. website analytics کاربرد رایج این window است.

## ۸. Stream joins

join در stream دشوارتر از join در batch است، چون یکی از streamها ممکن است بی‌نهایت باشد و رکورد متناظر هنوز نرسیده باشد. processor باید بخشی از state ورودی‌ها را نگه دارد و برای هر event جدید، آن state را query کند.
...
- event جست‌وجوی کاربر و click روی یکی از نتیجه‌ها
...
### Stream-table join یا stream enrichment
...
### Table-table join و materialized view

timeline خانگی شبکهٔ اجتماعی مثال خوبی است. اگر هنگام خواندن timeline مجبور باشیم همهٔ accountهای follow‌شده را پیدا کنیم و tweetهایشان را merge کنیم، query سنگین می‌شود. راه سریع‌تر ساختن inbox یا cache جدا برای هر user است.

eventهایی که این cache را به‌روز می‌کنند:
...
## ۹. Fault tolerance در stream processing
...
Spark Streaming از این ایده استفاده می‌کند. microbatch به‌طور ضمنی یک tumbling window بر اساس processing time هم ایجاد می‌کند. اگر window بزرگ‌تری لازم باشد، state باید از یک microbatch به بعدی منتقل شود.
...
- messageهای خروجی برای operatorهای بعدی
- messageهای خارجی، email یا push notification
- writeهای database
...
**Idempotency** یعنی اگر یک operation را چند بار اجرا کنیم، نتیجه با یک‌بار اجرا کردن آن یکی باشد.

- set کردن مقدار key به A، idempotent است؛ اجرای دوباره فقط همان A را می‌نویسد.
...
- replay باید همان پیام‌ها را با همان ترتیب بدهد.
- پردازش باید deterministic باشد.
- node دیگری نباید هم‌زمان همان مقدار را تغییر دهد.
...
windowed aggregation، counter، average، histogram و tableهای مورد استفاده در join همگی state دارند. این state بعد از crash باید recover شود.

چند راه:

1. **remote state**: state در database دور نگه‌داری و replicate شود. ساده است، اما query شبکه‌ای برای هر پیام می‌تواند کند باشد.
2. **local state با replication**: processor state را محلی نگه می‌دارد و تغییرها یا snapshotها را به storage پایدار می‌فرستد.
3. **checkpoint**: Flink snapshot state operator را در HDFS یا storage پایدار نگه می‌دارد.
4. **changelog topic**: Samza و Kafka Streams تغییرهای state را به topicای با log compaction می‌فرستند.
5. **پردازش redundant**: VoltDB یک input را روی چند node پردازش می‌کند تا state replica باشد.
6. **replay input**: اگر window کوتاه باشد، می‌توان eventهای همان window را دوباره خواند. local copy database نیز می‌تواند از compacted CDC stream دوباره ساخته شود.
...
stream processing، counterpart پیوستهٔ batch processing است: به‌جای ورودی محدود، stream بی‌پایان را می‌خواند و به‌جای خروجی نهایی، مرتب output جدید تولید می‌کند.

دو نوع broker اصلی داریم:

- **AMQP/JMS-style broker**: broker پیام‌ها را به consumerها تقسیم می‌کند، consumer برای هر پیام acknowledgment می‌دهد و پیام پس از تحویل حذف می‌شود. این مدل برای task queue و asynchronous RPC مناسب است.
- **log-based broker**: پیام‌ها در partitionهای پایدار روی disk می‌مانند، ترتیب داخل partition حفظ می‌شود، consumer با offset پیشرفت خود را ثبت می‌کند و هر زمان لازم باشد پیام‌های قدیمی را replay می‌کند. این مدل برای ساخت derived state و stream pipeline مناسب است.

stream می‌تواند از action کاربر، sensor، market feed یا writeهای database ساخته شود. CDC log تغییرهای database را در اختیار search index، cache و warehouse می‌گذارد. Event Sourcing، eventهای سطح domain را source of truth می‌کند و state فعلی را از replay آن‌ها می‌سازد.

کاربردهای مهم stream processing عبارت‌اند از:

- CEP برای پیدا کردن الگوی چند event
- analytics برای aggregate کردن داده در window
- materialized view برای به‌روز نگه‌داشتن representationهای read-optimized
...
- **orders**: سفارش جدید، لغو سفارش و برگشت کالا
- **payments**: پرداخت موفق یا ناموفق

یک stream processor می‌تواند این کارها را انجام دهد:

1. event سفارش را دریافت کند.
...
6. search index و dashboard را از stream موجودی به‌روز کند.
...
producer event را تولید و منتشر می‌کند. consumer event را می‌خواند و پردازش می‌کند.
...
### late event یا straggler

eventی که timestamp آن به window قدیمی تعلق دارد، اما به‌دلیل delay بعد از بسته شدن آن window می‌رسد.
...
snapshot پایدار از state پردازشگر که برای ادامهٔ کار بعد از failure استفاده می‌شود.
