...
2. batch و stream به‌عنوان دو روش ساختن derived data
3. unbundling database و ساختن application بر اساس dataflow
4. مشاهدهٔ state مشتق‌شده و اتصال آن به client
5. correctness سرتاسری، Idempotency و constraintها
6. timeliness، integrity و auditability
7. predictive analytics، privacy و مسئولیت اخلاقی
...
- data warehouse برای گزارش و analytics
- batch و stream processor برای محاسبهٔ metric
- cache یا نسخهٔ denormalized از objectها
...
update کردن derived system از روی event log را می‌توان deterministic و idempotent طراحی کرد. در این صورت، اگر consumer خراب شود، با replay و retry می‌توان state آن را دوباره ساخت.

### Derived data در برابر distributed transaction
...
distributed transaction معمولاً linearizability می‌دهد؛ یعنی بعد از commit، write برای readerها قابل‌مشاهده است و client می‌تواند انتظار read-your-writes داشته باشد. derived system معمولاً asynchronous است و چنین guarantee زمانی را به‌طور پیش‌فرض ندارد.

distributed transaction در محیط محدود ممکن است مفید باشد، اما XA معمولاً performance و fault tolerance ضعیف‌تری دارد و ترکیب آن با ابزارهای متفاوت دشوار است. به همین دلیل، log-based derived data در بسیاری از معماری‌ها گزینهٔ عملی‌تری است. البته نباید به کاربر گفت «eventual consistency اجتناب‌ناپذیر است و باید تحملش کنی»؛ باید برای guaranteeهایی مانند read-your-writes راه‌حل مشخص داشت.
...
- clientهای offline ممکن است بدون تأیید server تغییر بسازند و بعداً آن‌ها را sync کنند؛ در نتیجه client و server eventها را با ترتیب متفاوت می‌بینند.
...
- logical timestamp می‌تواند ترتیب مشترکی بسازد، اما consumerها همچنان باید eventهای out-of-order را مدیریت کنند.
- می‌توان eventی را که state دیده‌شدهٔ کاربر را ثبت می‌کند، با ID یکتا ذخیره کرد و eventهای بعدی را به آن ارجاع داد.
- conflict resolution برای state مفید است، اما side effect خارجی مانند ارسال notification را به‌تنهایی اصلاح نمی‌کند.
...
- Spark می‌تواند stream را با microbatchهای کوچک روی موتور batch پردازش کند.
- Apache Flink می‌تواند batch را به‌عنوان streamای محدود پردازش کند.
...
batch processing معمولاً به توابع deterministic و pure نزدیک است: input تغییر نمی‌کند، output در محل جدا نوشته می‌شود و side effect پنهان وجود ندارد. stream processing همین ایده را با state مدیریت‌شده و fault-tolerant گسترش می‌دهد.
...
1. input دریافت می‌شود.
2. function آن را transform می‌کند.
3. output در storage یا stream بعدی نوشته می‌شود.
4. consumer بعدی از آن استفاده می‌کند.
...
### Reprocessing برای evolution

stream تغییر تازه را با delay کم به view می‌رساند. batch می‌تواند حجم بزرگی از history را دوباره پردازش کند و view جدید بسازد. **Reprocessing** یکی از بهترین ابزارها برای evolution است:

- منطق جدید را روی dataset موجود اجرا می‌کنیم.
- index یا cache جدید را کنار نسخهٔ قدیمی می‌سازیم.
...
**Lambda architecture** پیشنهاد می‌کند eventهای immutable در یک dataset رو به رشد ذخیره شوند و دو pipeline جدا داشته باشیم:

- stream processor سریعاً view تقریبی را update می‌کند.
- batch processor بعداً همان eventها را با الگوریتم دقیق‌تر پردازش و view را اصلاح می‌کند.
...
- reprocess کردن کل history روی dataset بزرگ پرهزینه است.
- batch به incremental batch تقسیم می‌شود و به مسئلهٔ late event، window مرزی و time reasoning شبیه stream دچار می‌شود.
...
1. replay کردن eventهای قدیمی با همان engineای که eventهای جدید را پردازش می‌کند.
2. effectively-once semantics و دور ریختن output ناقص هنگام failure.
3. windowing بر اساس event time، نه processing time؛ چون هنگام reprocess کردن history، processing time معنایی ندارد.

Apache Beam نمونه‌ای از API است که می‌تواند چنین محاسبه‌هایی را برای engineهایی مانند Flink یا Google Cloud Dataflow بیان کند.

## ۳. Unbundling Databases
...
Unix و relational database از دو فلسفهٔ متفاوت آمده‌اند:

- Unix abstraction نسبتاً پایین و نزدیک به hardware می‌دهد: file و pipe دنباله‌ای از byte هستند.
- relational database abstraction بالاتری می‌دهد: SQL، index، query optimization، join، concurrency control و recovery را پنهان می‌کند.

Unix از نظر سادگی یعنی لایهٔ نازکی روی resourceها. database از نظر سادگی یعنی با یک query کوتاه از infrastructure قدرتمندی استفاده می‌کنیم. هیچ‌کدام همیشه بهتر نیستند.

### اجزای قابل ترکیب database

در فصل‌های قبل قابلیت‌های مختلف database را دیدیم:

- secondary index برای جست‌وجو بر اساس field
- materialized view به‌عنوان cache نتیجهٔ query
- replication log برای به‌روز کردن replicaها
- full-text index برای جست‌وجوی کلمه‌ها
...
4. writeهایی را که بعد از snapshot رخ داده‌اند پردازش می‌کند.
...
### Federated database: متحد کردن read

**Federated database** یا **polystore** یک query interface مشترک روی چند storage engine می‌سازد. PostgreSQL foreign data wrapper نمونه‌ای از این ایده است. application تخصصی می‌تواند مستقیماً به storage اصلی وصل شود، اما queryای که دادهٔ چند منبع را ترکیب می‌کند از interface فدرال استفاده می‌کند.

این روش روح relational دارد: یک query language سطح‌بالا و semantics یکپارچه. در عوض implementation آن پیچیده است.

### Unbundled database: متحد کردن write
...
CDC و event log را می‌توان unbundling قابلیت index maintenance دانست. این مدل به Unix نزدیک است:

- هر ابزار یک کار را خوب انجام می‌دهد.
- ابزارها با interface ساده، یعنی stream یا log، با هم ارتباط دارند.
- application یا یک زبان بالاتر آن‌ها را compose می‌کند.
...
event log هم ordering و durability کافی برای consistencyهای مهم دارد و هم به یک technology خاص محدود نیست.

### Unbundled در برابر integrated

unbundling قرار نیست databaseهای فعلی را حذف کند. database همچنان برای نگه‌داری state stream processor و پاسخ به queryهای output لازم است. data warehouse و engineهای MPP نیز برای workload تخصصی خود عالی‌اند.
...
Unix shell اجازه می‌دهد ابزارها را با pipe به‌سادگی به هم وصل کنیم. برای data system نیز می‌توان چنین زبان declarativeای تصور کرد:
...
## ۴. Designing Applications Around Dataflow
...
### Application code به‌عنوان derivation function

هر dataset مشتق‌شده با یک transformation function از dataset دیگری ساخته می‌شود:

- secondary index، fieldهای جدول اصلی را برمی‌دارد و بر اساس آن‌ها sort می‌کند.
- full-text index، language detection، word segmentation، stemming، spelling correction و synonym detection را انجام می‌دهد و inverted index می‌سازد.
- machine learning model از training data و feature extraction و تحلیل آماری مشتق می‌شود.
- cache، داده را در شکلی نگه می‌دارد که UI مستقیماً آن را نمایش دهد.

CREATE INDEX یک derivation function عمومی و استاندارد است. اما feature engineering مدل ML یا cache مخصوص UI به دانش application نیاز دارد. database trigger و stored procedure می‌توانند code اجرا کنند، ولی معمولاً برای این نوع integration طراحی اصلی database نیستند.

### جدا کردن application code از state

database می‌تواند user-defined function اجرا کند، اما برای dependency management، package، version control، rolling upgrade، metric، network call و اتصال به سیستم‌های بیرونی محیط مناسبی نیست. ابزارهایی مثل Docker، Kubernetes، Mesos و YARN برای اجرای application code ساخته شده‌اند.
...
2. consumer آن را می‌خواند.
3. consumer state یا view جدیدی را تغییر می‌دهد.
...
- actorها و queueهای memory-only ممکن است بعد از crash state و message را از دست بدهند.

modern stream processor باید ordering پایدار، delivery قابل‌اعتماد و state recovery را فراهم کند.

### Stream processor و service

microserviceها معمولاً با request/response و REST با هم حرف می‌زنند. stream operatorها با message stream یک‌طرفه و asynchronous compose می‌شوند.
...
## ۵. Observing Derived State

dataflow برای ساختن search index، materialized view و predictive model یک **write path** ایجاد می‌کند. داده از لحظهٔ جمع‌آوری، از چند مرحلهٔ batch و stream می‌گذرد تا همهٔ derived datasetها update شوند.
...
derived dataset مرز این دو مسیر است و trade-off را تعیین می‌کند: هرچه در write بیشتر precompute کنیم، read سریع‌تر می‌شود؛ هرچه write ساده‌تر باشد، read باید کار بیشتری انجام دهد.

![Figure 12-1: برخورد write path و read path در search index](../../assets/figures/ch12/figure-12-1.png)

### Materialized view و cache
...
cache و materialized view مرز write و read را جابه‌جا می‌کنند: بخشی از کار را زودتر انجام می‌دهیم تا پاسخ بعدی سریع شود. همان trade-off در مثال timeline شبکهٔ اجتماعی دیده شد؛ بعضی userها و celebrityها strategy متفاوتی می‌خواهند.

### Clientهای stateful و offline-capable
...
- UI و database محلی حتی بدون اینترنت کار می‌کنند.
...
### Push کردن state change به client
...
Elm و ابزارهایی مانند React، Flux و Redux state داخل client را با event stream مدیریت می‌کنند. می‌توان همین مدل را تا server ادامه داد:

1. action کاربر روی device اول event می‌سازد.
2. event از log و derived systemها عبور می‌کند.
3. state change به device دوم push می‌شود.
4. UI دوم بدون reload خودکار update می‌شود.
...
MPP database این قابلیت را آماده ارائه می‌کند و برای بسیاری از queryها انتخاب ساده‌تری است؛ اما query-as-stream راهی برای ساخت applicationهای بسیار بزرگ و خاص فراهم می‌کند.

## ۶. Aiming for Correctness
...
~~~sql
BEGIN TRANSACTION;
UPDATE accounts SET balance = balance + 11.00
  WHERE account_id = 1234;
UPDATE accounts SET balance = balance - 11.00
  WHERE account_id = 4321;
COMMIT;
~~~
...
### Operation ID و duplicate suppression
...
**End-to-end argument** می‌گوید بعضی functionها فقط وقتی کاملاً درست پیاده می‌شوند که application دو سر ارتباط در اجرای آن دخالت داشته باشد. لایهٔ پایین می‌تواند نسخه‌ای ناقص اما مفید ارائه کند، ولی guarantee نهایی در endpointها بررسی می‌شود.

TCP packetهای تکراری را داخل connection حذف می‌کند، database transaction را atomic می‌کند و stream processor retry را مدیریت می‌کند. اما هیچ‌کدام نمی‌فهمند user بعد از timeout همان POST را دستی دوباره فرستاده است. برای آن، operation ID باید از client تا مقصد نهایی عبور کند.
...
constraintهایی مانند uniqueness، non-negative balance، موجودی انبار و نبودن booking هم‌پوشان نیاز به تصمیم قطعی دارند. اگر چند request هم‌زمان مقدار یکسانی بخواهند، سیستم باید یکی را بپذیرد و بقیه را رد کند.
...
- request ID برای uniqueness همان ID
- username برای uniqueness نام کاربری
- seat ID برای رزرو صندلی

multi-master asynchronous برای uniqueness مناسب نیست، چون دو master ممکن است هم‌زمان مقدار متناقض را قبول کنند. اگر رد کردن فوری violation لازم باشد، synchronous coordination اجتناب‌ناپذیر است.

### Uniqueness در log-based messaging

log همهٔ consumerها را در یک partition به یک ترتیب می‌رساند. برای رزرو username:

1. هر درخواست به‌عنوان message و بر اساس hash username به partition خاص append می‌شود.
2. stream processor به‌صورت single-threaded آن partition را می‌خواند و در local database نام‌های گرفته‌شده را نگه می‌دارد.
3. اگر username آزاد باشد، آن را taken می‌کند و success event می‌فرستد.
4. اگر قبلاً گرفته شده باشد، rejection event می‌فرستد.
5. client output stream را دنبال می‌کند و پاسخ مربوط به request ID خود را می‌خواند.
...
1. client transfer را با request ID یکتا در log مربوط به همان ID ثبت می‌کند.
2. processor از این event دو instruction می‌سازد: debit برای account A و credit برای account B. هرکدام به partition account خودش می‌رود.
3. processorهای account instructionها را با request ID deduplicate می‌کنند و balance را تغییر می‌دهند.
...
**Timeliness** یعنی user state به‌روز را ببیند. اگر replica قدیمی باشد، user موقتاً مقدار قدیمی می‌خواند؛ با صبر یا retry ممکن است درست شود. linearizability راه قوی برای timeliness است، اما read-after-write نیز در بسیاری از کاربردها کافی است.

### Integrity

**Integrity** یعنی داده گم، خراب، متناقض یا نادرست نشود. اگر index از database مشتق شده باشد، باید همهٔ رکوردهای لازم را درست نشان دهد.
...
### Correctness در dataflow

ACID معمولاً timeliness و integrity را با هم می‌دهد. dataflow asynchronous این دو را از هم جدا می‌کند:

- timeliness فقط وقتی تضمین می‌شود که client برای پاسخ consumer منتظر بماند.
- integrity با delivery reliable، Idempotency، immutable event، operation ID و reprocessing حفظ می‌شود.

ترکیب مهم برای dataflow قابل‌اعتماد:

1. کل write کاربر را در یک message خودبسنده ثبت کنیم.
2. stateهای دیگر را با function deterministic از آن message بسازیم.
3. operation ID را در تمام stageها عبور دهیم.
4. messageها را immutable نگه داریم تا بعداً بتوانیم reprocess کنیم.

## ۸. Constraintهای انعطاف‌پذیر

uniqueness سخت به consensus و coordination نیاز دارد، اما همهٔ business constraintها به enforce لحظه‌ای نیاز ندارند.
...
1. dataflow می‌تواند integrity derived data را بدون atomic commit و synchronous cross-partition coordination حفظ کند.
...
coordination تعداد apologyها به‌علت inconsistency را کم می‌کند، اما ممکن است outage و latency را بیشتر کند. هدف حذف همهٔ apologyها نیست؛ هدف پیدا کردن trade-off مناسب است.

## ۹. Trust, but Verify

هر system model فرض می‌کند برخی چیزها ممکن است خراب شوند و برخی معمولاً درست می‌مانند:

- process می‌تواند crash کند.
- network ممکن است message را drop یا delay کند.
- disk ممکن است بعد از fsync داده را نگه دارد.
- memory و CPU معمولاً درست کار می‌کنند.
...
software هم bug دارد. حتی databaseهای معتبر ممکن است در edge caseهایی uniqueness یا serializable isolation را غلط اجرا کنند. application code معمولاً بیشتر در معرض bug است و گاهی constraintهایی را که database ارائه می‌دهد اشتباه استفاده می‌کند.
...
backup را نیز باید واقعاً restore کرد. وجود فایل backup کافی نیست؛ ممکن است backup ناقص یا غیرقابل‌بازگردانی باشد و تا روز بحران متوجه نشویم.
...
- processor را دوباره اجرا کرد و خروجی را با view فعلی مقایسه کرد.
- یک derivation مستقل و redundant برای بررسی ساخت.
- provenance و علت یک تصمیم را trace کرد.

dataflow deterministic نوعی time-travel debugging ایجاد می‌کند: می‌توان شرایط دقیق قبل از رخداد غیرمنتظره را بازسازی کرد.
...
cryptocurrency و distributed ledgerهایی مانند Bitcoin، Ethereum و دیگر سامانه‌ها ایده‌هایی برای audit رمزنگاری‌شده دارند: replicaهای مستقل integrity یکدیگر را بررسی می‌کنند و با consensus روی transactionها توافق می‌کنند. این فناوری‌ها trade-offها و محدودیت‌های خود را دارند؛ proof of work پرهزینه است و throughput برخی شبکه‌ها کم است، اما **Merkle tree** و proof عضویت برای audit عمومی جالب‌اند.
...
### Bias و discrimination

algorithm لزوماً از انسان عادل‌تر نیست. اگر input تاریخی discriminatory باشد، مدل همان bias را یاد می‌گیرد و گاهی تقویت می‌کند. ویژگی‌هایی مانند postal code یا IP address ممکن است proxy ویژگی‌های حفاظت‌شده مانند ethnicity یا class باشند.
...
### Responsibility و accountability
...
recommendation systemی که محتوای مورد علاقهٔ user را نشان می‌دهد، ممکن است user را در **echo chamber** قرار دهد و misinformation یا polarization را تقویت کند.
...
tracking می‌تواند برای feature مفید باشد: کلیک روی search result برای بهتر شدن ranking، پیشنهادهای مشابه، A/B test و تحلیل user flow. اما اگر مدل درآمدی بر advertisement باشد، advertiser مشتری اصلی می‌شود. tracking گسترده‌تر، profile دقیق‌تر و retention طولانی‌تر برای marketing ساخته می‌شود.
...
### Consent و آزادی انتخاب

گفتن این‌که user با terms of service موافقت کرده، الزاماً consent معنادار نیست:

- user معمولاً نمی‌داند چه داده‌ای جمع می‌شود و چه مدت نگه داشته می‌شود.
- privacy policy ممکن است بیشتر پنهان کند تا روشن سازد.
...
consent باید قابل فهم، قابل پس‌گرفتن و متناسب با استفادهٔ واقعی از داده باشد.

### Privacy و استفاده از داده

privacy به معنی پنهان کردن همه‌چیز نیست؛ یعنی هر شخص حق داشته باشد تصمیم بگیرد چه چیزی را به چه کسی نشان دهد و چه چیزی را مخفی نگه دارد. وقتی شرکت از زیرساخت surveillance برای استخراج داده استفاده می‌کند، این حق تصمیم‌گیری از فرد به شرکت منتقل می‌شود.
...
regulation بیش از حد ممکن است جلوی درمان یا پژوهش مفید را بگیرد، اما نبود regulation نیز privacy را تهدید می‌کند. تعادل فقط با قانون به‌دست نمی‌آید؛ صنعت باید culture خود را تغییر دهد:

- user را metric قابل‌بهینه‌سازی نبیند، بلکه انسان دارای dignity بداند.
...
- access control را تا حد امکان با protocol رمزنگاری‌شده enforce کند، نه فقط با promise سازمانی.
...
batch و stream مکمل‌اند: stream تغییرهای جدید را با latency کم منتقل می‌کند و batch history را دوباره پردازش و viewها را اصلاح یا بازسازی می‌کند.
...
unbundling database یعنی قابلیت‌های storage، index، replication، query و processing را به componentهای تخصصی تقسیم کنیم و با dataflow به هم وصل کنیم. این کار انعطاف می‌دهد، اما schema، backfill، monitoring، ownership و repair را نیز بر عهدهٔ application می‌گذارد.

correctness فقط وظیفهٔ database نیست. constraintهای مهم باید در مسیر end-to-end بررسی شوند. uniqueness به consensus نیاز دارد، اما بعضی business constraintها با validation بعدی و compensating action بهتر و مقیاس‌پذیرترند. timeliness و integrity را جدا کنید: دادهٔ کمی قدیمی ممکن است قابل‌قبول باشد، اما دادهٔ corrupted خطرناک است.
...
2. CDC تغییرها را به event log می‌فرستد.
3. stream processor پیشنهادهای تازه و dashboard را update می‌کند.
4. search index برای جست‌وجوی محصول ساخته می‌شود.
5. batch job هر شب history را reprocess می‌کند تا model recommendation اصلاح و backfill شود.
6. mobile app نسخهٔ محلی پیشنهادها را نگه می‌دارد و هنگام اتصال eventهای عقب‌افتاده را دریافت می‌کند.
...
2. derived data باید source، version، freshness و lineage قابل‌مشاهده داشته باشد.
3. batch و stream را بر اساس completeness و latency انتخاب کنید، نه بر اساس مد روز.
4. برای retryهای چندمرحله‌ای operation ID سرتاسری داشته باشید.
5. timeliness و integrity را جداگانه اندازه‌گیری و guarantee کنید.
6. constraint را در لایه‌ای enforce کنید که conflict را می‌بیند؛ constraintهای قابل‌جبران را بی‌دلیل linearizable نکنید.
7. backup، replay و audit را واقعاً تمرین کنید.
8. predictive model را از نظر bias، feedback loop و امکان appeal بررسی کنید.
9. retention و deletion را برای همهٔ derived systemها، نه فقط source، تعریف کنید.
...
### federated database و polystore

query interfaceای مشترک روی چند storage engine متفاوت؛ تمرکز اصلی آن متحد کردن read است.

### write path و read path

write path مسیر precompute کردن و آماده‌سازی derived data است. read path کاری است که هنگام query user انجام می‌شود. index و cache مرز این دو را جابه‌جا می‌کنند.
...
workflow چندمرحله‌ای بین سرویس‌ها که به‌جای یک rollback مشترک، برای هر مرحله action جبرانی تعریف می‌کند. saga atomicity لحظه‌ای ایجاد نمی‌کند، بلکه failure را به مراحل قابل‌مدیریت تقسیم می‌کند.

### RPO و RTO

RPO بیشترین data loss قابل‌قبول و RTO بیشترین زمان قابل‌قبول برای recovery را مشخص می‌کند.
